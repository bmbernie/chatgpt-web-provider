...
HOST_BRIDGE_CONTEXT = (
    ...
    "The DEVELOPER message above contains policy and runtime "
    ...
    "External Hermes host tools are NOT native ChatGPT tools and "
    "are not expected to appear in the native ChatGPT tool set or "
    "tool UI. Do not use the native ChatGPT tool set to decide "
    "whether a listed external tool is available.\n"
    "The external tool catalog supplied by this provider is the "
    "authoritative tool set for this request. Every tool listed "
    "there is available to the Hermes host. To invoke one, do not "
    "attempt a native ChatGPT tool call. Instead, emit the exact "
    "external tool-call envelope required by the provider. The "
    "provider will intercept that envelope and execute the tool "
    "through the Hermes host.\n"
    "If the user explicitly requests a listed external tool, use "
    "that external tool-call protocol instead of saying the tool "
    "is absent, unavailable, inaccessible, or missing from your "
    "actual/native tool set."
)
